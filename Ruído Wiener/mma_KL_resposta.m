...
for instante_atual = instantes
    ...
    KL1 = abs(divergencia_KL(histograma_x1,histograma_x2,largura_urnas));
    KL2 = abs(divergencia_KL(histograma_x2,histograma_x1,largura_urnas));

    KL(indice_KL) = min([KL1,KL2]);
    indice_KL = 1 + indice_KL;

end

hold on
semilogy(instantes,KL,LineWidth=3,Color=minhasCores(m_atual,:))
...
title(["$\mathbf{x}$ KL-Divergences for $\Delta t = 1$ s and $\zeta = 0.05$"],FontSize=20,Interpreter="latex")
xlabel("$t_1$ [s]",FontSize=20,Interpreter="latex")
ylabel("$\hat{\textrm{KL}}(\hat{p}_{\mathbf{x}_1},\hat{p}_{\mathbf{x}_2})$",FontSize=20,Interpreter="latex")