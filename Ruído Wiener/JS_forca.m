...
for instante_atual = instantes
    ...
    JS(indice_js) = divergencia_JS(histograma_f1,histograma_f2,largura_urnas);
    indice_js = 1 + indice_js;

end

hold on
semilogy(instantes,JS,LineWidth=3)
...
title(["$\mathbf{f}$ JS-Divergences for $\Delta t = 1$ s"],FontSize=20,Interpreter="latex")
xlabel("$t_1$ [s]",FontSize=20,Interpreter="latex")
ylabel("$\hat{\textrm{JS}}(\hat{p}_{\mathbf{f}_1},\hat{p}_{\mathbf{f}_2})$",FontSize=20,Interpreter="latex")